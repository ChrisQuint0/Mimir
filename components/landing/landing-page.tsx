            ...
            <header className="fixed top-0 w-full z-50 border-b border-slate-800/50 bg-slate-950/80 backdrop-blur-md">
                <div className="container flex h-16 items-center justify-between px-6 mx-auto max-w-7xl">
                    <div className="flex items-center gap-3 group cursor-pointer">
                        <Image src={logo} alt="Mimir Logo" width={40} height={40} className="transition-transform group-hover:scale-105" />
                        <span className="text-2xl font-bold tracking-tight text-white font-lora">Mimir</span>
                    </div>
                    ...
                    <div className="container px-6 mx-auto max-w-7xl relative">
                        <div className="flex flex-col items-center text-center space-y-10">
                            <h1 className="text-4xl md:text-5xl lg:text-6xl font-bold tracking-tight max-w-4xl balance leading-[1.15] animate-in fade-in slide-in-from-bottom-4 duration-1000 fill-mode-both text-white font-lora">
                                The Structure of a Bootcamp.<br />
                            ...
                            <p className="max-w-3xl text-lg md:text-xl text-slate-400 leading-relaxed animate-in fade-in slide-in-from-bottom-5 duration-1000 fill-mode-both delay-100 font-sans">
                                Tell Mimir what you want to learn and when. Our AI instantly generates a personalized, day-by-day curriculum with adaptive lessons and assessments designed specifically for your goals.
                            </p>
                ...
                <section id="features" className="py-24 border-t border-slate-800/50 bg-slate-900/20">
                    <div className="container px-6 mx-auto max-w-7xl">
                        <div className="mb-16 text-center">
                            <h2 className="text-3xl md:text-4xl font-bold text-white mb-4 font-lora">Pathways to Mastery</h2>
...
function SyllabusDemo({ user }: { user: User | null }) {
    ...
    const [activeLesson, setActiveLesson] = useState<{day: number, title: string, desc: string} | null>(null);
    ...
    return (
        <section ref={sectionRef} className="py-24 relative overflow-hidden flex flex-col items-center border-t border-slate-800/50">
            {/* Background glows */}
            <div className="absolute inset-0 overflow-hidden pointer-events-none">
                ...
                <div className="bg-slate-900/60 backdrop-blur-md rounded-3xl p-8 md:p-12 border border-slate-700/50 shadow-2xl shadow-blue-900/20 max-w-3xl mx-auto min-h-[400px]">
                    {step === "input" && (
                        <div className="space-y-8 animate-in fade-in zoom-in-95 duration-500">
                            ...
                            <div className="space-y-3 text-left">
                                ...
                                <div className="h-14 bg-slate-950 border border-slate-700 text-white text-lg rounded-xl flex items-center px-4 w-full cursor-text relative overflow-hidden">
                                    {typedText.length === 0 && !hasStartedTyping && (
                                        <span className="text-slate-500 absolute left-4 pointer-events-none">e.g., Master React Hooks, Learn Spanish...</span>
                                    )}
                                    <span className="z-10">{typedText}</span>
                                    <span className={`w-0.5 h-6 bg-blue-500 ml-[2px] ${hasStartedTyping ? 'animate-pulse' : 'hidden'}`}></span>
                                </div>
                            </div>
                            ...
                            <div className="pt-6 relative">
                                <button onClick={handleGenerate} className="w-full py-4 px-6 bg-[#6749fb] hover:bg-[#5a3ee0] text-white font-bold rounded-xl transition-all transform hover:scale-[1.02] active:scale-[0.98] shadow-xl shadow-[#6749fb]/40 flex items-center justify-center gap-3 group text-lg relative overflow-hidden">
                                    <Sparkles className="w-5 h-5 group-hover:rotate-12 transition-transform" />
                    ...
                    {step === "generating" && (
                        <div className="h-full min-h-[300px] flex flex-col items-center justify-center space-y-6 animate-in fade-in duration-500">
                            ...
                            <div className="space-y-2 text-center">
                                <h3 className="text-2xl font-bold text-white font-lora">Mimir is thinking...</h3>
                                <p className="text-slate-400">Crafting your personalized {duration[0]}-day journey</p>
                            </div>
                        </div>
                    )}
                    ...
                    {step === "result" && !activeLesson && (
                        <div className="space-y-6 animate-in slide-in-from-bottom-8 fade-in duration-700">
                            <div className="flex items-center justify-between border-b border-slate-700/50 pb-4">
                                <div>
                                    <h3 className="text-2xl font-bold text-white">Master React Hooks</h3>
                                    <p className="text-blue-400 font-medium text-sm mt-1">{duration[0]}-Day Curriculum</p>
                                </div>
                                <button onClick={() => { setStep("input"); setTypedText(""); setHasStartedTyping(false); }} className="text-sm text-slate-400 hover:text-white transition-colors flex items-center gap-1 group bg-slate-800/50 hover:bg-slate-800 px-3 py-1.5 rounded-lg border border-slate-700">
                            ...
                            <div className="space-y-3 max-h-[300px] overflow-y-auto pr-2 custom-scrollbar text-left">
                                {[
                                    { day: 1, title: "Understanding State & Effect", desc: "Deep dive into useState and useEffect core mechanics." },
                                    { day: 2, title: "Context API & Reducers", desc: "Managing complex state using useContext and useReducer." },
                                    { day: 3, title: "Custom Hooks & Optimization", desc: "Extracting logic and optimizing with useMemo/useCallback." },
                                    { day: 4, title: "Advanced Patterns & Refs", desc: "Exploring useRef for DOM manipulation and keeping mutable state." },
                                ].map((lesson) => (
                                    <div key={lesson.day} onClick={() => setActiveLesson(lesson)} className="p-4 rounded-xl bg-slate-800/40 border border-slate-700/50 hover:border-blue-500/30 transition-colors cursor-pointer group">
                                        <div className="flex items-center gap-4">
                                            <div className="size-10 rounded-lg bg-blue-500/10 text-blue-400 flex items-center justify-center font-bold">
                                                {lesson.day}
                                            </div>
                                            <div>
                                                <h4 className="font-semibold text-white group-hover:text-blue-400 transition-colors">{lesson.title}</h4>
                                                <p className="text-sm text-slate-400 mt-1 line-clamp-1">{lesson.desc}</p>
                                            </div>
                                        </div>
                                    </div>
                                ))}
                            ...
                            <div className="pt-2 flex w-full">
                                <Link 
                                    href={user ? `/dashboard?goal=Master React Hooks&duration=${duration[0]}` : `/login?goal=Master React Hooks&duration=${duration[0]}`} 
                                    className="w-full"
                                >
                                    <button className="w-full py-4 px-6 bg-[#6749fb] hover:bg-[#5a3ee0] text-white font-bold rounded-xl transition-all flex items-center justify-center gap-2 group">
                                        Start Learning For Real <ArrowRight className="w-5 h-5 group-hover:translate-x-1 transition-transform" />
                                    </button>
                                </Link>
                            </div>
                        </div>
                    ...
                    {step === "result" && activeLesson && (
                        <div className="space-y-6 animate-in slide-in-from-right-8 fade-in duration-500 text-left">
                            <div className="flex items-center justify-between border-b border-slate-700/50 pb-4">
                                <div>
                                    <h3 className="text-2xl font-bold text-white">Day {activeLesson.day}: {activeLesson.title}</h3>
                                </div>
                                <button onClick={() => setActiveLesson(null)} className="text-sm text-slate-400 hover:text-white transition-colors flex items-center gap-1 group bg-slate-800/50 hover:bg-slate-800 px-3 py-1.5 rounded-lg border border-slate-700">
                                    <ArrowLeft className="size-4 group-hover:-translate-x-1 transition-transform" /> Back
                                </button>
                            </div>
                            ...
                            <div className="max-h-[300px] overflow-y-auto pr-4 custom-scrollbar text-slate-300 space-y-4 text-sm leading-relaxed">
                                {activeLesson.day === 1 && (
                                    <>
                                        <h4 className="text-white font-bold text-lg">Introduction</h4>
                                        <p>React components need to remember things. <code>useState</code> is the hook that lets you add React state to functional components.</p>
                                        <h4 className="text-white font-bold text-lg mt-6">Core Mechanics</h4>
                                        <p>When state changes, the component re-renders. <code>useEffect</code> is used to synchronize your component with an external system or perform side effects.</p>
...
function Counter() {
  ...
  return <button onClick={() => setCount(c => c + 1)}>{count}</button>;
                                ...
                                {activeLesson.day === 2 && (
                                    <>
                                        <h4 className="text-white font-bold text-lg">Introduction</h4>
                                        <p>As applications grow, passing props down multiple levels becomes tedious. The Context API allows you to share data globally, and <code>useReducer</code> lets you manage complex updates predictable.</p>
                                        <h4 className="text-white font-bold text-lg mt-6">Core Mechanics</h4>
                                        <p>A reducer function takes the current state and an action, returning the new state. This provides a clear, maintainable state architecture.</p>
                                        <div className="bg-slate-950 p-4 rounded-xl border border-slate-800 font-mono text-xs text-blue-300 my-4 overflow-x-auto custom-scrollbar">
...
function Counter() {
  ...
  return <button onClick={() => dispatch({ type: 'increment' })}>{state.count}</button>;
                                ...
                                {activeLesson.day === 3 && (
                                    <>
                                        <h4 className="text-white font-bold text-lg">Introduction</h4>
                                        <p>When you have logic that needs to be shared across multiple components, you can extract it into a Custom Hook—a normal JavaScript function whose name starts with "use".</p>
                                        <h4 className="text-white font-bold text-lg mt-6">Core Mechanics</h4>
                                        <p>Additionally, React provides <code>useMemo</code> to cache expensive calculations, and <code>useCallback</code> to keep function references stable and prevent unnecessary child re-renders.</p>
                                        <div className="bg-slate-950 p-4 rounded-xl border border-slate-800 font-mono text-xs text-blue-300 my-4 overflow-x-auto custom-scrollbar">
...
// In component: const posts = useFetch('/api/posts');`}</code></pre>
                                        </div>
                                    </>
                                )}
                                {activeLesson.day === 4 && (
                                    <>
                                        <h4 className="text-white font-bold text-lg">Introduction</h4>
                                        <p>Sometimes you need to store information without triggering a re-render, or you need direct access to a DOM node. This is where <code>useRef</code> shines.</p>
                                        <h4 className="text-white font-bold text-lg mt-6">Core Mechanics</h4>
                                        <p>Unlike state, mutating the <code>.current</code> property of a ref doesn't re-render the component. It's an escape hatch that perfectly complements your React toolkit.</p>
                                        <div className="bg-slate-950 p-4 rounded-xl border border-slate-800 font-mono text-xs text-blue-300 my-4 overflow-x-auto custom-scrollbar">
                                            <pre><code>{`function TextInputWithFocus() {
